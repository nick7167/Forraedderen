 ...
 * nothing else.
 *
 * The travel is worth the pixels: this game is played by passing a phone around a table
 * and stabbing at controls, often by someone who has had a drink, and a control that
 * visibly moves confirms the hit without waiting for Convex to round-trip.
 *
 * `--press-edge` is set inline per variant and consumed by the `.press` utility in
 * index.css, which also drops the travel under prefers-reduced-motion.
 */
...
const VARIANTS: Record<ButtonVariant, { className: string; edge: string }> = {
  ...
  // same thing is the point — pressing this is how the round moves.
  primary: {
...
const SIZES: Record<ButtonSize, string> = {
  ...
  // except `sm`, which is only ever used inline next to text.
...
export function Button({
  ...
  return (
    <button
      ...
      // slow connection — which in this game means a duplicate clue or a double vote.
