...
export function SettingsCoach({
  ...
  return (
    <div
      ...
      onClick={onDismiss}
    >
      {/* Glowing gear over the real one (top-right of the TopBar). */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onOpenSettings();
        ...
        className="absolute top-1.5 right-3 flex size-9 items-center justify-center rounded-full bg-primary text-primary-foreground ring-2 ring-primary ring-offset-2 ring-offset-background animate-pulse gloss"
      >
        <Settings2 className="size-5" />
      </button>

      {/* Bubble pointing up at the gear. */}
      <div className="absolute top-14 right-3 flex flex-col items-end duration-300 animate-in fade-in slide-in-from-top-2">
